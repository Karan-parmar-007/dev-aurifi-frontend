<script lang="ts">
	...
	import { TransactionsystemColumnModal } from '../../store/toogleModal.svelte';
	...
	$effect(() => {
		if (TransactionsystemColumnModal.isTransactionSystemColumnModalOpen) {
	...
	async function saveEdit(event: Event) {
		...
		try {
			...
			const response = await fetch(
				`${VITE_API_URL}/admin/update_system_transaction_column/${columnId}`,
				{
					method: 'PUT',
					headers: { 'Content-Type': 'application/json' },
			...
			if (!response.ok) {
				...
				throw new Error(`Error updating system transaction column: ${error.message || error}`);
			}

			const data = await response.json();
			console.log('API response:', data);
			await fetchData();
			TransactionsystemColumnModal.isTransactionSystemColumnModalOpen = false; // Close modal only on success
		} catch (error) {
			...
			isLoading = false;
			if (!errorMessage) {
				// Only reset form if no_TER error occurred
				resetForm();
			}
		}
	}
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			console.log(
				...
				`${VITE_API_URL}/admin/add_system_transaction_column`,
				payload
			);
			const response = await fetch(`${VITE_API_URL}/admin/add_system_transaction_column`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
			...
			if (!response.ok) {
				const error = await response.json();
				throw new Error(`Error adding system transaction column: ${error.message || error}`);
			}

			const data = await response.json();
			console.log('API response:', data);
			await fetchData();
			TransactionsystemColumnModal.isTransactionSystemColumnModalOpen = false;
			resetForm();
		} catch (error) {
			console.error('Submission error:', error);
...
<Modal
	bind:open={TransactionsystemColumnModal.isTransactionSystemColumnModalOpen}
	title="System Transaction Column"
>
	{#if isLoading}
