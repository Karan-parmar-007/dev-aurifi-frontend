<script lang="ts">
	import { Card, Dropzone, Button, Input, Label, Select } from 'flowbite-svelte';
	import { modal, transactionModal } from '../../store/toogleModal.svelte';
	...
	let selectedPrimaryAsset = $state(''); // Default to empty
	let selectedSecondaryAsset = $state(''); // Default to empty
	let transactionName = $state(''); // State for project name input
	let assetClasses = $state([]); // State for fetched asset classes
	...
	onMount(async () => {
		try {
			...
			if (result.status === 'success') {
				assetClasses = result.data.map((asset) => ({
					value: asset.name,
					name: asset.name
				}));
			} else {
			...
			alert('Error fetching asset classes: ' + error.message);
		}
	});

	const handleCreateProject = async () => {
		// Validation: Check if file, project name, and at least one asset class are provided
		if (
			files.length === 0 ||
			!transactionName ||
			(!selectedPrimaryAsset && !selectedSecondaryAsset)
		) {
			alert('Please upload a file, provide a project name, and select at least one asset class!');
			return;
		}

		// Validation: Check if both asset classes are the same
		if (
			selectedPrimaryAsset &&
			selectedSecondaryAsset &&
			selectedPrimaryAsset === selectedSecondaryAsset
		) {
			alert('Primary and Secondary asset classes cannot be the same!');
			return;
		}
		...
		try {
			...
			formData.append('file', files[0]); // Use original file without renaming
			console.log('uploading file: ', files[0]);
			console.log('primary_asset_class', selectedPrimaryAsset);
			console.log('secondary_asset_class', selectedSecondaryAsset);

			formData.append('user_id', user_id); // User ID from constants
			formData.append('transaction_name', transactionName); // Project name from input
			formData.append('primary_asset_class', selectedPrimaryAsset); // Primary asset class
			formData.append('secondary_asset_class', selectedSecondaryAsset); // Secondary asset class

			// Send to backend
			const response = await fetch(`${VITE_API_URL}/transaction/upload_dataset`, {
				method: 'POST',
				body: formData
			});
			...
			if (!response.ok) {
				throw new Error(result.message || 'Failed to upload file to backend');
			}

			console.log('File uploaded successfully!', result);
			await invalidate('files'); // Invalidate to refresh data
			...
			transactionModal.isTransactionModalOpen = false;
		} catch (error) {
...
<Card class="relative flex min-w-[40%] flex-col gap-4 rounded-3xl border-none p-4 shadow-none">
	{#if isloading}
	...
	{#if files.length === 0}
		<span class="flex justify-between">
			...
			<button
				...
				onclick={() => (transactionModal.isTransactionModalOpen = false)}
			...
			>
				...
				>
					...
					/>
					<path
						d="M10.4243 9.57574C10.6586 9.81005 10.6586 10.1899 10.4243 10.4243L6.60589 14.2426C6.37157 14.477 5.99167 14.477 5.75736 14.2426C5.52304 14.0083 5.52304 13.6284 5.75136 13.3941L9.15147 10L5.75736 6.60589C5.52304 6.37157 5.52304 5.99167 5.75736 5.75736C5.99167 5.52304 6.37157 5.52304 6.60589 5.75736L10.4243 9.57574ZM9.125 9.4H10V10.6H9.125V9.4Z"
						fill="black"
					/>
				</svg>
			</button>
		...
		<p class="text-sm">
			Upload a CSV or XLSX file containing your transaction data. Make sure the format is per RBI
		...
		<Dropzone
			...
			accept=".csv, .xls, .xlsx, application/vnd.ms-excel, application
	...
	{:else}
		...
		<div class="flex flex-col gap-4">
			<div class="flex flex-col gap-2">
				<Label class="text-md font-normal text-black">Project Name</Label>
				<div class="flex items-center justify-center gap-4">
					<Input
						placeholder="Transaction 01"
						bind:value={transactionName}
						class="w-full rounded-md border-none"
					/>
					<span class="flex w-full overflow-hidden rounded-md bg-[#F8F8F8]">
						<!-- <img src="/excel.png" width="50"  alt="Excel icon" /> -->
						<span class="w-full rounded-md border-none bg-[#F8F8F8] p-2 px-4">
							{files[0].name}
						</span>
					</span>
				</div>
				<div class="flex gap-4">
					<Label class="text-md min-w-[50%] max-w-[50%] font-normal text-black"
						>Primary Asset Class</Label
					>
					<Label class="text-md min-w-[50%] max-w-[50%] font-normal text-black"
				...
				<div class="flex items-center justify-center gap-4">
					<Select
						id="primary-asset"
						class="mt-2"
						bind:value={selectedPrimaryAsset}
						placeholder="Select Primary Asset"
					>
						{#each assetClasses as { value, name }}
					...
					<Select
						id="secondary-asset"
						class="mt-2"
						bind:value={selectedSecondaryAsset}
			...
			<div class="  w-full rounded-xl bg-[#FDF8EF] p-4">
				<p class="text-[14px] text-[#6B480D]">
					Select at least one asset class. If selecting both asset classes, ensure they are
					different and there is a column named Asset Class in the sheet.
				</p>
			</div>
		</div>
		<div class="button-group flex items-center justify-center gap-2">
			...
			>
			<Button color="alternative" class="hover:text-black" onclick={() => (files = [])}>
				Reupload
			</Button>
		</div>
	{/if}
