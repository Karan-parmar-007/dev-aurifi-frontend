<script lang="ts">
	...
	async function handleDelete(id: string, columnName: string) {
		if (confirm(`Are you sure you want to delete column "${columnName}"?`)) {
			try {
				const response = await fetch(`${VITE_API_URL}/admin/delete_system_column/${id}`, {
					method: 'DELETE',
					headers: {
...
<div class="flex w-full flex-col items-center justify-center">
	<div class="h-[80%] w-[92%] py-10">
		...
		{#if systemColumnModal.isSystemColumnModalOpen === true}
			<AdminAddColumnForm {editingColumn} {purpose} {fetchData} {cleanup} />
		{/if}
	</div>
</div>
